...
session_start();
	...
	if ($resultado_id) {
		...
		while ($registro = mysqli_fetch_array($resultado_id, MYSQLI_ASSOC)) { // APENAS INDICES ASSOCIATIVOS.
			...
			echo '<button type="button" id="btn-seguir_'.$registro['id'].'" class="btn btn-default btn-seguir" data-id_usuario="'.$registro['id'].'">Seguir</button>';
			echo '<button type="button" id="btn-deixar_seguir_'.$registro['id'].'" style="display:none" class="btn btn-primary btn-deixar_seguir" data-id_usuario="'.$registro['id'].'">Deixar de Seguir</button>';
			echo '</p>';
